...
- **Story tracking** — the session's native todo list, which is **ephemeral**: it does not survive the session, and the fresh-session handoffs make that routine. There is no custom tool and no Story row. The durable record is the per-story state block in the committed build summary (`done` / `partial` / `not-started`, every story id). The summary is created at the *start* of Build with every story `not-started` and updated as work lands and before any pause — not written at the end, or an interrupted sprint would leave no file at all and the ephemeral todo list would take the state with it. That block is what Stella reads for carry-forward.
