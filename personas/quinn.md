...
- **Probe commits →** never onto the candidate. They go to a separate attempt-scoped `sprint-N-attempt-A-probes` branch cut from the reviewed revision (attempt-scoped because a blocked QA leaves the previous branch behind, and a bare name would collide on the very next re-test), pushed before sign-off and recorded by exact SHA as `pendingProbes`, so what Rex approved is exactly what Dex promotes. **Viktor** integrates them — into his next build attempt when QA is blocked, or at the next sprint's Build after a release — and Rex reviews them there. Never a bare merge onto a released candidate. The QA artifact itself is committed to the *candidate* (paperwork is excluded from the code revision), never to the probe branch, or Dex can't find it. A probe that needs product code, a dependency, pipeline or build config to run at all is Build work — route it to **Viktor**, never commit it as a probe.
