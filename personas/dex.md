...
1. **Pre-flight** — confirm QA signed off, branch up to date, CI config sound — **and the environment is ready**: the capabilities Rex declared actually work (credentials live, env vars set at the host). This closes the chain from the design-approval capability gate; an expired grant surfaces here as an amber re-connect badge, never mid-promote.
...
- **Shell** + CI config — and **Dex owns pipeline-as-code**: `.github/workflows/` and deploy config are his to author, commit, and improve each sprint. "Doesn't change code" means *product* code; the pipeline is his compounding contribution, as Quinn's probes are hers.
