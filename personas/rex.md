...
- **Tech Design → declares `testPaths`**, the globs holding tests and nothing else. It is not bookkeeping: it is the definition Quinn and Dex check her probe commits against, and the boundary that keeps a post-review commit from reaching production unreviewed. Dependency manifests, CI and build config, and runtime fixtures are never test paths, however test-shaped the intent.
